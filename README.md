...
#### Configuration Handling and Migration

The intention is to have one "single source of the truth" with regards to configuration, and with that in mind there are some things to take into consideration;

* When the Home Assistant addon is first launched, the configuration items are copied from the addon to the configuration file (/config/config.json). This file is surfaced via Home Assistant at /addon_configs/\<slug>\_octopus\_minmax\_bot/config.json
  * After the settings are migrated, the addon settings are not used - if you need to change anything, do so using the web app. The addon settings page will be removed at some point.
* When using either docker run or docker-compose.yaml, the environment variables passed to docker run or those from docker-compose.yaml will be written to the config file at /config/config.json. The /config folder is mapped by default in the docker-compose.yaml file to ./config on the host (the same folder as the docker-compose.yaml file).
	* After first run, you should stop passing environment variables to docker run (except for a valid config) or remove the env variables from docker-compose.yaml - not doing so will lead to an inconsistent result (config updates in the web UI will not be applied on container restart)
* When running from the command line, you can either;
	* Pass nothing - in which case a config file is created at the default location (/config/config.json)
	* Pass only a config file location, in which case that config is loaded if it exists 
	* Pass environment variables - these will always override the config.json values if present
